...
a <- datos_clima %>% 
  mutate(precipitation = case_when(
    daily_precipitation_sum < 24 ~ "Baja",
    daily_precipitation_sum >= 24 | daily_precipitation_sum <= 48 ~ "Media",
    daily_precipitation_sum > 48 ~ "Alta",
  ))

b <- a %>% 
  select(sitio, fecha = date, precipitation) %>% 
  right_join(registros_por_camara_por_especie_por_dia, by = join_by(sitio, fecha)) %>% 
  group_by(sp)
